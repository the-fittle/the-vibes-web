...
{
    private _styles: JSX.CSSProperties = {};

    /** Returns the current styles as a CSS object */
    styles (): JSX.CSSProperties
    {
        return this._styles
    }

    /** Add a single CSS property to the styles */
    ...
    /** Combine the current Modifier with another Modifier */
    then ( modifier?: Modifier ): Modifier
    {
        if ( !modifier ) return this

        const combined = new Modifier()
        combined._styles = { ...this._styles, ...modifier._styles }
        return combined
    }

    /** Combine conditionally */
    thenIf ( condition: any | undefined, modifier?: Modifier ): Modifier
    {
        return condition && condition != undefined ? this.then( modifier ) : this
    }
    ...
    // Typography
    // -----------------------------------------------------------------------------------------------------------
    typography = ( typography: Typography ): Modifier =>
    {
        console.log( typography )

        this.thenIf( typography[ 'size' ], this.fontSize( typography[ 'size' ] ) )
        this.thenIf( typography[ 'family' ], this.fontFamily( typography[ 'family' ] ) )
        this.thenIf( typography[ 'weight' ], this.fontWeight( typography[ 'weight' ] ) )
        this.thenIf( typography[ 'textAlign' ], this.textAlign( typography[ 'textAlign' ] ) )
        this.thenIf( typography[ 'textDecoration' ], this.textDecoration( typography[ 'textDecoration' ] ) )
        this.thenIf( typography[ 'color' ], this.color( typography[ 'color' ] ) )
    ...
    textDecoration = ( decoration: JSX.CSSProperties[ 'text-decoration' ] ): Modifier =>
        this.css( "text-decoration", decoration );
    ...
    margin (
        value: JSX.CSSProperties[ 'margin' ]
    ): Modifier
    margin (
        values: Partial<{ top: JSX.CSSProperties[ 'top' ]; right: JSX.CSSProperties[ 'right' ]; bottom: JSX.CSSProperties[ 'bottom' ]; left: JSX.CSSProperties[ 'left' ] }>
    ): Modifier
    margin (
        ...args: [ JSX.CSSProperties[ 'margin' ] ] | [ Partial<{ top: JSX.CSSProperties[ 'right' ]; right: JSX.CSSProperties[ 'bottom' ]; bottom: JSX.CSSProperties[ 'left' ]; left: JSX.CSSProperties[ 'top' ] }> ]
    ): Modifier
    {
        if ( args.length === 1 )
        {
            ...
            {
                throw new Error( "Invalid argument: expected a string for single argument." )
            }
        } else if ( args.length === 1 && typeof args[ 0 ] === "object" )
        {
            ...
            if ( !values )
                throw new Error( "Invalid argument: object values cannot be undefined." )

            if ( values.top )
                this.marginTop( values.top )

            if ( values.right )
                this.marginRight( values.right )

            if ( values.bottom )
                this.marginBottom( values.bottom )

            if ( values.left )
                this.marginLeft( values.left )
    ...
    padding (
        value: JSX.CSSProperties[ 'padding' ]
    ): Modifier
    padding (
        values: Partial<{ top: JSX.CSSProperties[ 'top' ]; right: JSX.CSSProperties[ 'right' ]; bottom: JSX.CSSProperties[ 'bottom' ]; left: JSX.CSSProperties[ 'left' ] }>
    ): Modifier
    padding (
        ...args: [ JSX.CSSProperties[ 'padding' ] ] | [ Partial<{ top: JSX.CSSProperties[ 'right' ]; right: JSX.CSSProperties[ 'bottom' ]; bottom: JSX.CSSProperties[ 'left' ]; left: JSX.CSSProperties[ 'top' ] }> ]
    ): Modifier
    {
        if ( args.length === 1 )
        {
        ...
        } else if ( args.length === 1 && typeof args[ 0 ] === "object" )
        {
            ...
            if ( values.top )
                this.paddingTop( values.top )

            if ( values.right )
                this.paddingRight( values.right )

            if ( values.bottom )
                this.paddingBottom( values.bottom )

            if ( values.left )
                this.paddingLeft( values.left )
    ...
    top = ( value: JSX.CSSProperties[ 'top' ] ): Modifier =>
        this.css( "top", value );
    ...
    fillParent = (): Modifier =>
        this.position( "absolute" ).fillMaxSize();

    fillViewport = (): Modifier =>
        this.position( "fixed" ).fillMaxSize();
    ...
    border ( value: JSX.CSSProperties[ 'border' ], color: JSX.CSSProperties[ 'border-color' ] ): Modifier
    border ( value: JSX.CSSProperties[ 'border' ], color: JSX.CSSProperties[ 'border-color' ], radius: JSX.CSSProperties[ 'border-radius' ] ): Modifier
    border ( value: JSX.CSSProperties[ 'border' ], color: JSX.CSSProperties[ 'border-color' ], radius: JSX.CSSProperties[ 'border-radius' ], width: JSX.CSSProperties[ 'border-width' ] ): Modifier
    border (
    ...
    outline ( value: JSX.CSSProperties[ 'outline' ], color: JSX.CSSProperties[ 'outline-color' ] ): Modifier
    outline ( value: JSX.CSSProperties[ 'outline' ], color: JSX.CSSProperties[ 'outline-color' ], width: JSX.CSSProperties[ 'outline-width' ] ): Modifier
    outline (
    ...
    shadow = ( options: {
        ...
        this.css( "box-shadow", createShadow( options ) )

    elevation = ( z: number, color: string = "rgba(0, 0, 0, 0.118)" ): Modifier =>
        this.css( "box-shadow", createShadow( { x: 0, y: z, blur: z, color } ) )
...
export function createShadow ( options: ShadowOptions ): string
{
    return `${ options.x || 0 }px ${ options.y || 0 }px ${ options.blur || 0 }px ${ options.spread || 0 }px ${ options.color || "rgba(0, 0, 0, 0.118)" }`
}

/** Create a drop-shadow string */
...
export function createDropShadow ( options: DropShadowOptions ): string
{
    return `${ options.x || 0 }px ${ options.y || 0 }px ${ options.blur || 0 }px ${ options.color || "rgba(0, 0, 0, 0.118)" }`
}

export interface Typography
{
    size?: JSX.CSSProperties[ 'font-size' ]
    family?: JSX.CSSProperties[ 'font-family' ]
...
export class Alignment extends Modifier
{
    static Center = (): Alignment =>
        mod().css( "place-items", "center" );

    static Start = (): Alignment =>
        mod().css( "place-items", "start" );

    static End = (): Alignment =>
        mod().css( "place-items", "end" );

    static Stretch = (): Alignment =>
        mod().css( "place-items", "stretch" );

    static Baseline = (): Alignment =>
        mod().css( "place-items", "baseline" );
}
...
export class Arrangement extends Modifier
{
    static Center = (): Arrangement =>
        mod().css( "place-content", "center" );

    static Start = (): Arrangement =>
        mod().css( "place-content", "start" );

    static End = (): Arrangement =>
        mod().css( "place-content", "end" );
    ...
    static SpacedBy = ( value: Fp | Sp ): Arrangement =>
        mod().css( "gap", value );
}
...
export class Shape extends Modifier
{
    ...
    static Squircle = ( roundness: number = 0.5 ): Shape =>
    {
        ...
        const squirclePath = `M ${ points[ 0 ] } ${ points.slice( 1 ).map( ( p ) => `L ${ p }` ).join( ' ' ) } Z`
...
// Shadow
// -----------------------------------------------------------------------------------------------------------
export class Shadow extends Modifier
{
    static Small = (): Shadow =>
        mod().shadow( {
            x: 0,
    ...
    static Box = ( options: ShadowOptions ): Shadow =>
        mod().shadow( options );

    static DropShadow = ( options: DropShadowOptions ): Shadow =>
        mod().css( "filter", `drop-shadow(${ createDropShadow( options ) })` );
}